...
class Args():
    def __init__(self):
        ...
        self.model_name_or_path = args.base_model
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, args, df, block_size=512, evaluate=False, strategy=True):
        ...
        if evaluate:
            cached_features_file = os.path.join(
                directory, args.model_type + "_dev_cached_lm_" + str(block_size) + args.mode
            )
        else:
            cached_features_file = os.path.join(
                directory, args.model_type + "_train_cached_lm_" + str(block_size) + args.mode
            )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            self.features = []
            for idx, row in tqdm(enumerate(df[:-1])):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating",disable=True):
        ...
        with torch.no_grad():
            ...
            if not args.turn:
                turn_ids = None
            if args.strategy:
                outputs = model(input_ids, decoder_input_ids=decoder_input_ids, decoder_turn_ids=decoder_turn_ids,
                                decoder_role_ids=decoder_role_ids, turn_ids=turn_ids, role_ids=role_ids,
                                labels=decoder_label_ids)
                loss = ppl = outputs.loss
            else:
...
def main(args):
    ...
    config = BlenderbotSmallConfig.from_pretrained(args.model_name_or_path)
    tokenizer = BlenderbotSmallTokenizer.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir)
    tokenizer.add_tokens(additional_special_tokens)
    tokenizer.add_special_tokens({'cls_token': '[CLS]'})
    model = BlenderbotSmallForConditionalGeneration.from_pretrained(
        args.model_name_or_path,
        from_tf=False,
        config=args.model_name_or_path+"/config.json",)
    ...
    if args.do_train:
        ...
        tokenizer = BlenderbotSmallTokenizer.from_pretrained(args.output_dir)
        # tokenizer.add_tokens(additional_special_tokens)
        # tokenizer.add_special_tokens({'cls_token': '[CLS]'})
        # model.resize_token_embeddings(len(tokenizer))
        model.to(args.device)
...
def generate_and_evaluate(args):
    ...
    write_path = "./metric/predict_480.json"
    print("write result to:", write_path)
    with open(write_path, "w",encoding="utf-8") as f:
        json.dump(refs,f,indent=2,ensure_ascii=False)
    with open("./metric/ref_480.json","w",encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name_or_path', default='/data/pretrained_models/blender_small',
                        help='path of PLMs')
    parser.add_argument('--data_path', default='./dataset', help='path of datasets')
    parser.add_argument('--train_file_name', default='trainWithStrategy_short.tsv',
                        help='train file')
    parser.add_argument('--eval_file_name', default='devWithStrategy_short.tsv',
                        help='test file')
    parser.add_argument('--cache_dir', default = 'cached', help = 'cache file')

    parser.add_argument('--learning_rate', type=float, default=5e-5, metavar='LR', help='learning rate')
    parser.add_argument('--per_gpu_train_batch_size', type=int, default=16, metavar='BS', help='batch size')
    parser.add_argument('--per_gpu_eval_batch_size', type=int, default=8, metavar='EBS', help='eval batch size')
    parser.add_argument('--gradient_accumulation_steps', type=int, default=2, metavar='GAS',
                        help='gradient accumulation steps')
    parser.add_argument('--weight_decay', type=float, default=0.0, metavar='WD', help='weight decay')
    parser.add_argument('--adam_epsilon', type=float, default=1e-8, metavar='AE', help='adam epsilon')
    parser.add_argument('--max_grad_norm', type=float, default=1.0, metavar='MGN', help='max grad norm')
    parser.add_argument('--block_size', type=int, default=512, metavar='BLS', help='block size')
    parser.add_argument('--num_train_epochs', type=int, default=5, metavar='E', help='number of epochs')
    parser.add_argument('--max_steps', type=int, default=-1, metavar='MS', help='max steps')
    parser.add_argument('--warmup_steps', type=int, default=120, metavar='WS', help='warmup steps')
    parser.add_argument('--logging_steps', type=int, default=30, metavar='LOS', help='logging steps')
    parser.add_argument('--save_steps', type=int, default=120, metavar='SS', help='save steps')
    parser.add_argument('--seed', type=int, default=42, metavar='SEED', help='seed')
    parser.add_argument('--local_rank', type=int, default=-1, metavar='LOR', help='local rank')
    parser.add_argument('--num_return', type=int, default=1, metavar='NUM', help='num_return')

    parser.add_argument('--do_train', action='store_true', default=True, help='train')
    parser.add_argument('--do_eval', action='store_true', default=True, help='eval')
    parser.add_argument('--generation', action='store_true', default=False, help='not know')
    parser.add_argument('--generate_and_eval', action='store_true', default=False, help='generate and eval')
    parser.add_argument('--evaluate_during_training', action='store_true', default=True,
                        help='evaluate during training')
    parser.add_argument('--eval_all_checkpoints', action='store_true', default=False, help='eval all checkpoints')
    parser.add_argument('--no_cuda', action='store_true', default=False, help='no cuda')
    parser.add_argument('--save_total_limit', action='store_true', default=False, help='save total limit')

    parser.add_argument('--overwrite_output_dir', action='store_true', default=True, help='overwrite output dir')
    parser.add_argument('--overwrite_cache', action='store_true', default=False, help='not overwrite_cache')
    parser.add_argument('--strategy', action='store_true', default=True, help='not strategy')

    parser.add_argument('--should_continue', action='store_true', default=False, help='not should_continue')
    parser.add_argument('--fp16', action='store_true', default=False, help='not fp16')
    parser.add_argument('--turn', action='store_true', default=False, help='not turn')
    parser.add_argument('--role', action='store_true', default=False, help='not role')

    parser.add_argument('--model_type', default='baseline', help='saved name, self defined')
    parser.add_argument('--mode', default='train', help='mode')
    parser.add_argument('--output_dir', default='checkpoint-480',
                        help='saved models')
